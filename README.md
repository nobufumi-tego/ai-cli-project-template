...
このテンプレートは「動くものを作る」だけでなく、**現代のソフトウェア開発で実際に使われる習慣とツール**を体験しながら学ぶことを意図して設計されています。

---

### 1. 仕様を先に書く習慣（AGENTS.md）

`scripts/init-project.py` を実行すると、まず **AGENTS.md**（プロジェクト仕様書）を作ります。

> コードを書く前に「何を作るか・どう作るか」を言葉にする。

この習慣は、プロのエンジニアが設計書・要件定義書を書くのと同じ考え方です。
AI CLI を使わなくても、AGENTS.md はチームメンバーや将来の自分へのドキュメントとして機能します。
...
`pyproject.toml` と `uv.lock` により、**どのパソコンでも同じ環境**を再現できます。

> 「自分のパソコンでは動くのに、他の人のパソコンでは動かない」を防ぐ。

「環境構築ができない」は開発者が最初に直面する壁のひとつです。
uv を使うことで、その壁を低く乗り越える経験ができます。
...
ファイルを保存するたびに **Ruff（リンター）** が自動で実行されます（`.claude/settings.json` の Hook 設定）。

> 「動けばいい」から「読みやすく・壊れにくいコードを書く」へ。

型ヒント・docstring・定数化などのルールは、最初は面倒に感じますが、
コードが大きくなるほど「書いていてよかった」と実感できるようになります。
...
AI に「丸投げ」するのではなく、**仕様書（AGENTS.md）を渡して一緒に作る**スタイルを体験できます。

> AI はツール。何を作りたいかを伝えられる人間が主導する。

AGENTS.md に規約・構成・注意事項を育てていくことで、AI の出力の質が上がります。
「AI に何をどう伝えればよいか」を考える力は、これからの時代に不可欠なスキルです。
...
| 上級 | AI CLI と協働して開発サイクルを回せる | Claude Code・カスタムコマンド |
