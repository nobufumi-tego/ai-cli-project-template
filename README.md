...
**Python は不要です。** uv が Python を自動管理するため、事前インストール不要です。

必要なものは以下の 2 つだけです：

| ツール | 用途 | 備考 |
|---|---|---|
| **git** | リポジトリのクローン | 多くの環境に標準搭載 |
| **uv** | Python 管理・パッケージ管理 | curl / PowerShell でインストール |
...
**macOS**
```bash
xcode-select --install   # Xcode コマンドラインツール（git を含む）
```
...
> Python がインストールされていない場合、uv が初回実行時に自動でダウンロードします。
> 手動でインストールしたい場合は `uv python install 3.12` を実行してください。
